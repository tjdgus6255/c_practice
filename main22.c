...
typedef struct {
	int data;
	int height; // 높이를 저장해서 시간 복잡도를 보장할 수 있음
...
void setHeight(Node* node) {
	...
	// 왼쪽자식노드의 높이와 오른쪽자식노드의 높이중 더 큰 값에 1을 더한 높이가
	// node의 실제 높이
}

int getDifference(Node* node) { // 균형 인수
	if (node == NULL) return 0;
...
Node* rotateLL(Node* node) {
	...
	// 왼쪽자식노드에는 기존 왼쪽자식의 오른쪽자식노드를 삽입
	leftChild->rightChild = node;
	// 기존 왼쪽자식노드의 오른쪽자식노드로 node를 삽입
	setHeight(node); // 회전 이후에 높이를 다시 계산
	return leftChild; // leftChild는 이후에 setHeight() 수행
}

Node* rotateRR(Node* node) { // rotateLL의 반대
	Node* rightChild = node->rightChild;
